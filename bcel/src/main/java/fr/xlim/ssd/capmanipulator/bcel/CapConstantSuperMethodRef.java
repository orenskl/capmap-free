...
public class CapConstantSuperMethodRef extends CapConstantPoolInfo {
	private ClassRef classRef;
	private byte token;

	public CapConstantSuperMethodRef(ConstantSuperMethodRef superMethodRef) throws UnableToReadCapFileException {
		super(ConstantPoolComponent.TAG_SUPER_METHOD_REF);
		setClassRef(superMethodRef.getAssociatedClass());
	}
	...
	public byte[] getPackageAID() throws Exception {
		if(classRef instanceof ExternalClassRef) {
			byte packageToken = (byte) (((ExternalClassRef) classRef).getPackageToken() & 0x7F);

			return Tools.List2Array(CapFileFactory.getCapFile().getImportComponent().getPackages().get((int) packageToken).getAID());
		}
		if(classRef instanceof InternalClassRef) {
			return Tools.List2Array(CapFileFactory.getCapFile().getHeaderComponent().getThePackage().getAID());
		}
		throw new Exception("Error on the ClassRef");
	}
